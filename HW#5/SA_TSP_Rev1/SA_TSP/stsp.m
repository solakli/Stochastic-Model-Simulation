...
%      singleton*.
%
%      H = STSP returns the handle to a new STSP or the handle to
%      the existing singleton*.
%
%      STSP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STSP.M with the given input arguments.
%
%      STSP('Property','Value',...) creates a new STSP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stsp_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stsp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stsp
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @stsp_OpeningFcn, ...
                   'gui_OutputFcn',  @stsp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stsp is made visible.
function stsp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stsp (see VARARGIN)

% Choose default command line output for stsp
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes stsp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stsp_OutputFcn(hObject, eventdata, handles) 
